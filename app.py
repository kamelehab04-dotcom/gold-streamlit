# ============================================================
# BLACK PYRAMID v2003 – PRECISION EDITION (Fixed KeyError)
# ============================================================
...
def detect_smc_ict_advanced(df):
    ...
    df['choch_bullish'] = df['swing_low'].shift(1) & (df['close'] > df['high'].shift(1))
    df['choch_bearish'] = df['swing_high'].shift(1) & (df['close'] < df['low'].shift(1))
    return df
...
# -------------------- CALCULATE ALL INDICATORS (FIXED) --------------------
def calculate_all_indicators(df):
    if df is None or len(df) < 20:   # الحد الأدنى 20 صف بدلاً من 100
        return df
    df = df.copy()
...
# -------------------- PRECISION SIGNAL GENERATOR (FIXED) --------------------
def generate_precision_signal(df, symbol, dxy_signal=None, dxy_correlation=0.0,
    ...
    # Daily Bias (مع فحص وجود الأعمدة)
    daily_bias = "NEUTRAL"
    if df_daily is not None and len(df_daily) > 20:
        daily_last = df_daily.iloc[-1]
        if all(col in daily_last.index for col in ['ema20','ema50','rsi']):
    ...
    if (daily_bias == "BULLISH" and setup_type == "BUY") or (daily_bias == "BEARISH" and setup_type == "SELL"):
        factors['bias'] += 15.0
        details['Bias'] = "✅ Aligned with Daily"
    else:
        details['Bias'] = "⚠️ Neutral or Opposite"
    ...
    atr_ratio = last.get('atr', 10) / df['atr'].iloc[-20:].mean() if len(df)>=20 and df['atr'].iloc[-20:].mean()>0 else 1.0
    ...
    if setup_type == "BUY" and total_score >= 40:
        signal = "BUY"
        confidence = min(95, 60 + total_score * 0.6)
    elif setup_type == "SELL" and total_score >= 40:
        signal = "SELL"
    ...
    # Stop Loss & Targets
    atr_val = last.get('atr', 10) if not pd.isna(last.get('atr', 10)) else 10.0
    ...
    if signal == "BUY":
        targets = {'target1': entry_price + risk, 'target2': entry_price + risk * 1.8, 'target3': entry_price + risk * 3.0, 'risk_reward': 2.5}
    else:
        targets = {'target1': entry_price - risk, 'target2': entry_price - risk * 1.8, 'target3': entry_price - risk * 3.0, 'risk_reward': 2.5}
...
# -------------------- BACKTEST --------------------
def run_backtest_precision(df, symbol, lookback=500):
    ...
    for i in range(100, len(test_df) - 1):
            ...
            if result:
                ...
                if not active.get('breakeven_triggered', False):
                    if active['direction'] == "BUY" and bar['high'] >= active['tp1']:
                        active['stop'] = active['entry']; active['breakeven_triggered'] = True
        ...
        active = {
            ...
            'tp1': tp1, 'tp2': tp2, 'tp3': tp3, 'entry_i': i+1,
...
with st.sidebar:
    ...
    st.caption("London/NY Session | CHOCH + OB/FVG | Daily Bias")
    ...
    if st.session_state.all_signals is not None and not st.session_state.all_signals.empty:
        ...
        st.dataframe(df_sig[["Instrument","Signal","Confidence","Setup","Bias","DXY"]], hide_index=True, use_container_width=True, height=400)
        st.caption(f"🕐 {st.session_state.last_update.strftime('%H:%M:%S')} | {len(df_sig)} Setups")
    else:
        st.info("No high-probability setups.")
...
if signal in ["BUY","SELL"] and conf >= 55:
    ...
    st.warning("⏳ No high-probability setup currently.")
...
st.caption(f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | BLACK PYRAMID v2003 - Precision Mode (Fixed)")
