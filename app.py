...
# ==========================================
# Calculate Indicators (DEFINE FUNCTIONS FIRST)
# ==========================================
...
if order_block_bullish:
    bullish += 3
    strong_buy_conditions += 1
    signals.append("🏛️ ORDER BLOCK BULLISH")
if order_block_bearish:
    bearish += 3
    strong_sell_conditions += 1
    signals.append("🏛️ ORDER BLOCK BEARISH")
...
if signal_action == "BUY":
    ...
    rr_ratio = (targets[0] - entry) / (entry - stop_loss) if (entry - stop_loss) > 0 else 0
elif signal_action == "SELL":
    entry = current_price
    stop_loss = resistance + (current_atr * 0.8)
    lot = risk_manager.get_recommended_lot(entry, stop_loss)
    targets = [support, support - (current_atr * 1.5), support - (current_atr * 3)]
    rr_ratio = (entry - targets[0]) / (stop_loss - entry) if (stop_loss - entry) > 0 else 0
else:
...
if signal_action == "BUY":
    ...
    <div class="signal-card signal-buy">
        ...
        <div>Confidence: {confidence}% | {strong_buy_conditions} strong factors</div>
        <div style="margin-top:10px; font-size:0.9rem">Risk: 1% | Stop: ${stop_loss:.2f}</div>
    </div>
    """, unsafe_allow_html=True)
elif signal_action == "SELL":
    st.markdown(f"""
    <div class="signal-card signal-sell">
        ...
        <div>Confidence: {confidence}% | {strong_sell_conditions} strong factors</div>
        <div style="margin-top:10px; font-size:0.9rem">Risk: 1% | Stop: ${stop_loss:.2f}</div>
    </div>
    """, unsafe_allow_html=True)
else:
    ...
    <div class="signal-card signal-neutral">
        ...
        <div>Confidence: {confidence}% | No clear signal</div>
...
if signal_action != "NEUTRAL":
    ...
    <div class="targets-table">
        ...
        <div class="target-row"><span class="target-label">Lot Size</span><span class="target-value">{lot:.2f} lots</span></div>
