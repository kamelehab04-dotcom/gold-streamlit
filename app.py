...
def calculate_position_size(pair_name, entry, stop, balance, risk_percent):
    """
    Unified risk-money model.
    This does NOT pretend every asset has the same pip value.
    Contract/tick specifications are explicitly asset dependent.
    For live brokerage execution, broker-specific tick value should
    replace the fallback assumptions.
    """
    ...
    if asset == "forex":
        # Approximate USD-account pip model.
        ...
        # GC=F is a futures reference, not necessarily the user's broker CFD.
        # Treat as a configurable dollar-risk approximation.
        contract_size = 100.0
        loss_per_lot = distance * contract_size
...
# ============================================================
# BLACK PYRAMID v2004.1 – Adaptive Asset Scoring & Thresholds
# ============================================================
V2004_VERSION = "2004"
V2004_ASSET_PROFILES = {
    "FOREX": {
        "weights": {
            "trend": 0.25,
            "momentum": 0.20,
...
def v2004_asset_class(symbol):
    s=str(symbol).upper()
    if any(x in s for x in ("BTC","ETH","XRP","SOL","ADA","DOGE","BNB")): return "CRYPTO"
    if any(x in s for x in ("XAU","GOLD")): return "GOLD"
    return "FOREX"

def v2004_score(pillar_scores: dict, asset_class: str) -> dict:
    profile = V2004_ASSET_PROFILES[asset_class]
    w = profile["weights"]

    clean = {
        k: max(0.0, min(100.0, float(pillar_scores.get(k, 50.0))))
        for k in w
    }
    score = sum(clean[k] * w[k] for k in w)

    # A pillar between 45 and 55 is treated as NEUTRAL, not as a conflict.
    bull = sum(w[k] for k in w if clean[k] > 55)
    bear = sum(w[k] for k in w if clean[k] < 45)
    conflict = min(bull, bear) * 100.0

    t = profile["thresholds"]
    if conflict > t["max_conflict"]:
        state = "WAIT"
    elif score >= t["strong"]:
        state = "STRONG"
    elif score >= t["normal"]:
        state = "NORMAL"
    elif score >= t["developing"]:
        state = "DEVELOPING"
    else:
        state = "WAIT"
...
def v2004_directional_signal(bull_score,bear_score,asset_class,rr):
    t=V2004_ASSET_PROFILES[asset_class]["thresholds"]; spread=abs(float(bull_score)-float(bear_score))
    if rr<t["min_rr"] or spread<8: return "WAIT"
    if bull_score>bear_score: return "STRONG BUY" if bull_score>=t["strong"] and rr>=t["strong_rr"] else "BUY"
    return "STRONG SELL" if bear_score>=t["strong"] and rr>=t["strong_rr"] else "SELL"


# ===== v2004.1 CLEAN UI =====
V20041_UI_CSS="""<style>
.stApp{background:#0b0f14}.block-container{max-width:1450px;padding-top:1.2rem}.bp-header{padding:18px 22px;border:1px solid rgba(255,255,255,.08);border-radius:16px;background:#111821;margin-bottom:16px}.bp-title{font-size:30px;font-weight:800}.bp-subtitle{color:#9aa7b5;font-size:14px;margin-top:5px}div[data-testid="stMetric"]{background:#111821;border:1px solid rgba(255,255,255,.08);padding:10px 14px;border-radius:12px}div[data-testid="stSidebar"]{background:#0e141c;border-right:1px solid rgba(255,255,255,.08)}.stButton>button{border-radius:10px;font-weight:700}.bp-footer{color:#9aa7b5;text-align:center;padding:18px 0 4px;font-size:11px}
</style>"""
def render_v20041_shell():
    st.markdown(V20041_UI_CSS,unsafe_allow_html=True)
    st.markdown('<div class="bp-header"><div class="bp-title">BLACK PYRAMID <span style="color:#d6b36a">v2004.1</span></div><div class="bp-subtitle">Adaptive Market Intelligence • Multi-Timeframe • Risk-Aware</div></div>',unsafe_allow_html=True)
def render_v20041_metrics(signal="WAIT",score=0,rr=0,asset="FOREX"):
    c1,c2,c3,c4=st.columns(4)
    c1.metric("SIGNAL",signal); c2.metric("CONFIDENCE",f"{float(score):.1f}%"); c3.metric("RISK / REWARD",f"{float(rr):.2f}"); c4.metric("ASSET",asset)
def render_v20041_layout():
    with st.sidebar:
        st.markdown("### BLACK PYRAMID")
        st.caption("v2004.1")
        return st.radio("Navigation",["Dashboard","Analysis","Trade Plan","Risk Manager","Diagnostics","Settings"])
def render_v20041_footer():
    st.markdown('<div class="bp-footer">BLACK PYRAMID v2004.1.1 • Paper Trading / Decision Support</div>',unsafe_allow_html=True)
...
.main-title { font-size: 2.3rem; font-weight: 900; color: #ffd700; }
.main-subtitle { color: #aaa; margin-top: 6px; }
.signal-box {
    background:#11111c;
    padding:24px;
    border-radius:16px;
...
.signal-text { font-size:3rem; font-weight:900; }
.card {
    background:#11111c;
    padding:16px;
    border-radius:12px;
...
st.markdown(f"""
<div style="text-align:center;color:#888;padding:25px;border-top:1px solid #333;margin-top:30px;">
...
# ============================================================
# v2004.1 – Signal Diagnostics / Flexible Entry Engine
# ============================================================

def v20041_new_diagnostics():
    return {
        "score_rejected": 0,
        "conflict_rejected": 0,
        "rr_rejected": 0,
...
def v20041_count_supporting_pillars(pillar_scores: dict) -> int:
    return sum(
        1 for value in pillar_scores.values()
        if 55 < float(value) <= 100
    )
...
def v20041_signal(
    ...
    rr: float,
    mtf_ok: bool = True,
    high_impact_news_block: bool = False,
    diagnostics: dict | None = None,
) -> dict:
    if diagnostics is None:
        diagnostics = v20041_new_diagnostics()

    diagnostics["total_evaluations"] += 1
    profile = V2004_ASSET_PROFILES[asset_class]
    thresholds = profile["thresholds"]

    result = v2004_score(pillar_scores, asset_class)
    score = result["score"]
    conflict = result["conflict"]
    supporters = v20041_count_supporting_pillars(pillar_scores)

    # Diagnostics identify the first blocking reason without hiding other metrics.
    if high_impact_news_block:
        diagnostics["news_rejected"] += 1
        state = "WAIT"
        reason = "HIGH_IMPACT_NEWS"
    elif not mtf_ok:
        diagnostics["mtf_rejected"] += 1
        state = "WAIT"
        reason = "MTF_CONFLICT"
    elif conflict > thresholds["max_conflict"]:
        diagnostics["conflict_rejected"] += 1
        state = "WAIT"
        reason = "PILLAR_CONFLICT"
    elif float(rr) < thresholds["min_rr"]:
        diagnostics["rr_rejected"] += 1
        state = "WAIT"
        reason = "RR_TOO_LOW"
    elif score >= thresholds["strong"] and float(rr) >= thresholds["strong_rr"] and supporters >= 4:
        state = "STRONG"
        reason = "STRONG_CONFLUENCE"
        diagnostics["accepted_trades"] += 1
    elif score >= thresholds["normal"] and supporters >= 3:
        state = "NORMAL"
        reason = "QUALIFIED_ENTRY"
        diagnostics["accepted_trades"] += 1
    elif score >= thresholds["developing"] and supporters >= 2:
        state = "DEVELOPING"
    ...
    if state in ("NORMAL", "STRONG"):
        signal = f"{'STRONG ' if state == 'STRONG' else ''}{side}"
    else:
        signal = "DEVELOPING" if state == "DEVELOPING" else "WAIT"
    ...
    return {
        ...
        "score": score,
        "conflict": conflict,
        "supporting_pillars": supporters,
        "rr": round(float(rr), 2),
        "reason": reason,
        "diagnostics": diagnostics,
    }

def v20041_diagnostics_summary(diagnostics: dict) -> dict:
    total = max(1, int(diagnostics.get("total_evaluations", 0)))
    accepted = int(diagnostics.get("accepted_trades", 0))
    return {
        **diagnostics,
...
def render_v20041_diagnostics(diagnostics: dict):
    d = v20041_diagnostics_summary(diagnostics)
    st.markdown('<div class="bp-section">Signal Diagnostics</div>', unsafe_allow_html=True)
    cols = st.columns(6)
    labels = [
        ("Accepted", d["accepted_trades"]),
        ("Score Reject", d["score_rejected"]),
        ("Conflict", d["conflict_rejected"]),
        ("RR Reject", d["rr_rejected"]),
        ("MTF Reject", d["mtf_rejected"]),
        ("News Reject", d["news_rejected"]),
    ]
    for col, (label, value) in zip(cols, labels):
        col.metric(label, value)
    st.caption(
        f"Evaluations: {d['total_evaluations']} • "
        f"Acceptance rate: {d['acceptance_rate']}%"
    )
